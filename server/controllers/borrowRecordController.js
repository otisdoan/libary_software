...
class BorrowRecordController {
    ...
    async renewBorrowRecord(req, res) {
        try {
            const { extensionDays } = req.body;
            const result = await borrowRecordService.renewExpiredBorrowRecord(req.params.id, extensionDays);
            res.status(200).json({ message: 'Borrow record renewed successfully', result });
        } catch (error) {
            res.status(400).json({ message: error.message });
        }
    }

    async getExpiredBorrowRequests(req, res) {
        try {
            const page = Math.max(1, parseInt(req.query.page) || 1);
            const size = Math.min(100, Math.max(1, parseInt(req.query.size) || 10));
            const sortField = ['createdAt'].includes(req.query.field)
                ? req.query.field
                : 'createdAt';
            const result = await borrowRecordService.findAllExpired(page, size, sortField);
            res.status(200).json(result);
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
